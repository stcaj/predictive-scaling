...
def compute_epoch_durations(
    lines: List[str],
    start_marker: str = "Epoch 1/10...",
    end_marker: str = "Train accuracy",
) -> Dict[str, Any]:
    """
    """
...
def classify(v, CLASSIFICATION_VALUES):
    for i, val in enumerate(CLASSIFICATION_VALUES):
        if v <= val:
            return i
    return len(CLASSIFICATION_VALUES) - 1
    
# CLASSIFICATION_VALUES = [20, 40, 60, 80, 100]
def data_classification(dataset, CLASSIFICATION_VALUES):
    df = dataset.copy()
    df["class"] = df["cpu_percent"].apply(lambda v: classify(v, CLASSIFICATION_VALUES))
    return df
...
def offline_data_preprocess(
    ...
    df = dataset.copy()
    CLASSIFICATION_VALUES = metadata["data_param"]["class_values"]
    df_class = data_classification(df, CLASSIFICATION_VALUES)

    class_dist = df_class["class"].value_counts(normalize=True).sort_index()
    logs(f"Class distribution:\n{class_dist.to_string()}")

    df_ready = add_cyclical_feature(df_class, col="hour", period=24)
...
def offline_data_preprocess_for_testing(
    ...
    df = dataset.copy()
    # rovnaká klasifikácia ako pri tréningu
    CLASSIFICATION_VALUES = metadata["data_param"]["class_values"]
    df_class = data_classification(df, CLASSIFICATION_VALUES)
    df_ready = add_cyclical_feature(df_class, col="hour", period=24)
    df_ready = add_cyclical_feature(df_ready, col="minute", period=60)
