...
def build_model(seq_length: int, feature_count: int, units1, units2):
    '''Build LSTM model for multi-feature input'''
    model = Sequential()
    model.add(LSTM(units=units1, return_sequences=True, input_shape=(seq_length, feature_count)))
...
def main():
    ...
    model = build_model(seq_length=seq_length, feature_count=X.shape[1], units1=units1, units2=units2)
