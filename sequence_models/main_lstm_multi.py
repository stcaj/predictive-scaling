...
from my_modules.data_util import data_split_multi
...
def build_lstm_multi_model(seq_length: int, feature_count: int, units1, units2):
    '''Build LSTM model for multi-feature input'''
    model = Sequential([
        Input(shape=(seq_length, feature_count)),
    ...
    while True:
        ...
        try:
            ...
            if len(X) > 0 and len(y) > 0:
                ...
                if model is None:
                    model = build_lstm_multi_model(seq_length=seq_length, feature_count=X.shape[1], units1=units1, units2=units2)
                else:
