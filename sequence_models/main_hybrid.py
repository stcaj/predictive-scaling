...
def build_hybrid_model(seq_length: int, units):
    input_layer = Input(shape=(seq_length, 1))

    lstm_branch = LSTM(units)(input_layer)
    lstm_branch = Dropout(0.2)(lstm_branch)

    conv_branch = Conv1D(filters=units, kernel_size=3, activation='swish')(input_layer)
    ...
    output_layer = Dense(1)(x)   # linear output for regression
...
#     concat_layer = concatenate([lstm_branch, conv_branch, gru_branch])
#     output_layer = Dense(128)(concat_layer)
...
def main():
    ...
    while True:
        ...
        try:
            ...
            if model is None:
                model = build_hybrid_model(seq_length=seq_length, units=units)
            else:
